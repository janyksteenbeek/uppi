...
class ServerMonitoringControllerTest extends TestCase
{
    ...
    private function generateHmacSignature(string $payload, string $secret, int $timestamp = null): array
    {
    ...
    public function test_report_endpoint_requires_valid_hmac_signature(): void
    {
    ...
    public function test_report_endpoint_rejects_old_timestamps(): void
    {
        $server = Server::factory()->create();
        $payload = json_encode(['cpu_usage' => 50.0]);
        $oldTimestamp = time() - 400; // 6+ minutes ago
        
        $headers = $this->generateHmacSignature($payload, $server->secret, $oldTimestamp);

        $response = $this->postJson("/api/server/{$server->id}/report", json_decode($payload, true), $headers);
        $response->assertStatus(401);
    }

    public function test_report_endpoint_accepts_valid_basic_metrics(): void
    {
        $server = Server::factory()->create();
        $payload = json_encode([
            'cpu_usage' => 45.2,
        ...
        $headers = $this->generateHmacSignature($payload, $server->secret);
        ...
        $this->assertTrue($server->last_seen_at->greaterThan(now()->subMinute()));
    }

    public function test_report_endpoint_accepts_disk_metrics(): void
    {
        $server = Server::factory()->create();
        $payload = json_encode([
            'cpu_usage' => 50.0,
    ...
    public function test_report_endpoint_accepts_network_metrics(): void
    {
        $server = Server::factory()->create();
        $payload = json_encode([
            'cpu_usage' => 50.0,
    ...
    public function test_report_endpoint_calculates_disk_usage_percent_if_missing(): void
    {
        $server = Server::factory()->create();
        $payload = json_encode([
            'disk_metrics' => [
    ...
    public function test_report_endpoint_validates_required_fields(): void
    {
        $server = Server::factory()->create();
        $payload = json_encode([
    ...
    public function test_get_config_endpoint_requires_valid_hmac(): void
    {
        $server = Server::factory()->create();
        ...
        $headers = $this->generateHmacSignature('', $server->secret);
    ...
    public function test_cleanup_endpoint_requires_authentication(): void
    {
    ...
    public function test_cleanup_endpoint_deletes_old_metrics(): void
    {
        $user = User::factory()->create();
        ...
        $this->assertEquals(1, $responseData['deleted_count']);
    }

    public function test_report_endpoint_handles_nonexistent_server(): void
    {
        $payload = json_encode(['cpu_usage' => 50.0]);
        $headers = $this->generateHmacSignature($payload, 'fake-secret');
    ...
    public function test_report_endpoint_logs_successful_submissions(): void
    {
        ...
        // $this->assertLogged('info', 'Server metrics received successfully');
    }

    public function test_report_endpoint_uses_provided_collected_at_timestamp(): void
    {
        $server = Server::factory()->create();
        ...
        $metric = ServerMetric::where('server_id', $server->id)->first();
        $this->assertEquals('2025-01-20 10:30:00', $metric->collected_at->format('Y-m-d H:i:s'));
    }
}