...
<div class="space-y-2">
        ...
        <div class="flex items-start gap-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50">
            <div class="flex-shrink-0 mt-0.5">
                @if($check->status->value === 'ok')
                    @svg('phosphor-check-circle', 'w-5 h-5 text-success-500')
                @elseif($check->status->value === 'fail')
                    @svg('phosphor-x-circle', 'w-5 h-5 text-danger-500')
                @else
                    @svg('phosphor-question', 'w-5 h-5 text-warning-500')
                @endif
            </div>
    ...
    @empty
        <div class="text-center py-6 text-gray-500 dark:text-gray-400">
            @svg('phosphor-cell-signal-high', 'w-8 h-8 mx-auto mb-2 opacity-50')
